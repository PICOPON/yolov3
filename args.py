...
anchor_path = './data/my_data/yolo_anchors.txt'  # The path of the anchor txt file.
#class_name_path = './data/coco.names'  # The path of the class names.
class_name_path = './data/my_data/mydata.names'  # The path of the class names.
...
batch_norm_decay = 0.99  # decay in bn ops 0.9~0.99
weight_decay = 0.01  # L2 weight decay default value 5e-4 0.005 0.01
...
learning_rate_init = 0.01 #0.002 0.005
lr_type = 'cosine_decay'  # Chosen from [fixed, exponential, cosine_decay, cosine_decay_restart, piecewise]
...
lr_lower_bound = 1e-5  # The minimum learning rate.
...
warm_up_epoch = 50   # Warm up training epoches. Set to a larger value if gradient explodes.

### some constants in validation
# nms
nms_threshold = 0.1  # iou threshold in nms operation
score_threshold = 0.4  # threshold of the probability of the classes in nms operation, i.e. score = pred_confs * pred_probs. set lower for higher recall.
